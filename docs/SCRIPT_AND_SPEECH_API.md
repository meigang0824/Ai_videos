...
注意：导入音色不会覆盖旧音色，会生成新的音色记录。启用 OSS/MinIO 后，音频会直接上传到对象存储，不再写入持久化 `voices/` 目录，数据库保存 `object_key`；试听原音仍访问本系统接口，由后端从 OSS 流式返回音频。未启用对象存储时才会保存到本地 `voices/`，数据库里的 `ref_wav` 使用相对路径，避免迁移到云端后仍指向开发机绝对路径。
