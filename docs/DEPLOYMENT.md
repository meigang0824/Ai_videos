...
- 上传视频素材。
- 上传提取文案的原始媒体文件。
- 启用 OSS 后新导入的音色音频会直接进入对象存储；未启用 OSS 时才保存到本地 `voices/`。音色名称、归属、OSS key 等结构化信息会写入数据库。
- TTS 输出音频。
- 视频剪辑输出 MP4 和字幕 SRT。
- 口型同步输出 MP4。
...
如果上传时报 `SignatureDoesNotMatch`，优先检查 `.env` 里的 `ALIYUN_OSS_ACCESS_KEY_ID` 和 `ALIYUN_OSS_ACCESS_KEY_SECRET` 是否与阿里云控制台完全一致，不能多空格、引号或换行。修改 `.env` 后需要重建并重启容器：
