...
- `cosy_postgres`：PostgreSQL 数据库，保存用户、任务和上传记录。
- `cosy_storage`：服务配置和兼容本地 SQLite 回落文件。
- `cosy_outputs`：音频、视频、字幕输出。
- `cosy_voices`：本地音色录音和元数据。
...
- Docker 数据：当前 Docker Compose 使用 named volumes，PostgreSQL 数据在 `cosy_postgres`，服务配置在 `cosy_storage`。只复制代码不会带走这些 volume。
...
如果是 Docker named volume 生产部署，还需要额外备份/恢复 PostgreSQL：
